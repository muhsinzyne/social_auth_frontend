    ...
    >
      <div class="flex items-center justify-center mt-8">
        <div class="flex items-center">
          ...
          <span class="mx-1 text-2xl font-semibold text-white"
            >Social Auth</span
          >
        </div>
      </div>
      ...
      <nav class="mt-10">
        <div
          ...
          :class="[
            $route.name === tab.name.toLowerCase()
              ? activeClass
              : inactiveClass,
          ]"
          @click="handleNavigate(tab.name.toLowerCase())"
        >
          <div v-html="tab.icon"></div>
          <span class="mx-4">{{ tab.name }}</span>
        </div>
      </nav>
    </div>
  </div>
